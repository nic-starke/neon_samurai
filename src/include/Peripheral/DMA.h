...
typedef struct {
  DMA_CH_t*           pChannel;
  uint8_t             Repeats; // <= 1 for single shot mode
  uint16_t            BytesPerTransfer;
 ...
 */
static inline DMA_CH_t* DMA_GetChannelPointer(uint8_t ChannelNumber) {
  return (DMA_CH_t*)((uintptr_t)(&DMA.CH0) +
                     (sizeof(DMA_CH_t) * ChannelNumber));
}
 ...
 */
static inline bool DMA_ChannelBusy(uint8_t ChannelNumber) {
  volatile uint8_t busy = DMA.STATUS;
...
/**
 * @brief Set the DMA transaction source/start address for a specific DMA
 * channel.
 *
 * @param pDMA A pointer to a memory mapped DMA channel.
 * @param Address The address to set the DMA transaction to.
 */
static inline void DMA_SetChannelSourceAddress(DMA_CH_t* pDMA,
                                               uintptr_t Address) {
  pDMA->SRCADDR0 = (Address >> 0) & 0xFF;
  pDMA->SRCADDR1 = (Address >> 8) & 0xFF;
  // pDMA->SRCADDR2 = (Address >> 16) & 0xFF;
  pDMA->SRCADDR2 = 0; // On xmega this must be explicitly set to zero otherwise
                      // DMA transactions will go out-of-bounds (runaway).
}
 ...
 */
static inline void
DMA_SetChannelInterrupts(DMA_CH_t*          pDMA,
                         eInterruptPriority TransactionIntPriority,
                         eInterruptPriority ErrorIntPriority) {
  CLR_REG(pDMA->CTRLB, DMA_CH_ERRINTLVL_gm | DMA_CH_TRNINTLVL_gm);
  SET_REG(pDMA->CTRLB, (ErrorIntPriority << DMA_CH_ERRINTLVL_gp) |
                           (TransactionIntPriority << DMA_CH_TRNINTLVL_gp));
}
