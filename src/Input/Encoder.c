...
/**
 * @brief Checks if a hardware encoder has been rotated - updates an encoder
 * state based on this.
 *
 * @param EncoderIndex The hardware encoder index.
 * @param pHardwareEncoder A pointer to the hardware encoder state.
 * @param pEncoderState A pointer to the encoder state (from one of the virtual
 * banks)
 * @return True if a hardware encoder has been rotated - false otherwise.
 */
static inline bool UpdateEncoderRotary(int               EncoderIndex,
                                       sHardwareEncoder* pHardwareEncoder,
                                       sEncoderState*    pEncoderState) {
  pEncoderState->PreviousValue = pEncoderState->CurrentValue;

  uint8_t dir  = Encoder_GetDirection(EncoderIndex);
  int16_t move = 0;
...
#if (ACC_ENABLED == 1)
    pHardwareEncoder->CurrentVelocity =
        move * Acceleration(pHardwareEncoder, move, pVE->FineAdjust);
#else
    pHardwareEncoder->CurrentVelocity = move * 1000;
#endif
    int32_t newValue = (int32_t)pEncoderState->CurrentValue +
                       pHardwareEncoder->CurrentVelocity;
...
/**
 * @brief Updates the switch state for an encoder based on the current switch
 * hardware state
 *
 * @param EncoderIndex The hardware encoder index to check.
 * @param pEncoderState A pointer to the encoder state to be updated.
 */
static inline void UpdateEncoderSwitch(int            EncoderIndex,
                                       sEncoderState* pEncoderState) {
  // Process Encoder Switch
  if (pEncoderState->Switch.Mode != SWITCH_DISABLED) {
    pEncoderState->Switch.State =
        (bool)EncoderSwitchCurrentState(SWITCH_MASK(EncoderIndex));
    ...
    case SWITCH_LAYER_TOGGLE: {
      if (EncoderSwitchWasPressed(SWITCH_MASK(EncoderIndex))) {
        pEncoderState->Layers[pEncoderState->Switch.ModeParameter.Value]
            .Enabled =
            !pEncoderState->Layers[pEncoderState->Switch.ModeParameter.Value]
                 .Enabled;
      }
    ...
    case SWITCH_LAYER_CYCLE: {
      if (EncoderSwitchWasPressed(SWITCH_MASK(EncoderIndex))) {
        ...
        pEncoderState
            ->Layers[pEncoderState->Switch.ModeParameter.LayerTransition
                         .NextLayer]
            .Enabled = true;
        pEncoderState->Switch.ModeParameter.LayerTransition.CurrentLayer =
      ...
      //         uint8_t nextLayer =
      //         (pEncoderState->Switch.ModeParameter.LayerTransition.CurrentLayer
      //         + 1) % NUM_VIRTUAL_ENCODER_LAYERS;
      //         pEncoderState->Layers[pEncoderState->Switch.ModeParameter.LayerTransition.CurrentLayer].Enabled
      //         = false; pEncoderState->Layers[nextLayer].Enabled = true;
      //         pEncoderState->Switch.ModeParameter.LayerTransition.CurrentLayer
      //         = nextLayer;
      //         pEncoderState->Switch.ModeParameter.LayerTransition.NextLayer =
      //         (nextLayer + 1) % NUM_VIRTUAL_ENCODER_LAYERS;
      //     }
      //     break;
      // }
...
static inline void ProcessEncoderLayers(sEncoderState* pEncoderState) {
  for (int layer = 0; layer < NUM_VIRTUAL_ENCODER_LAYERS; layer++) {
    ...
    if (IsLayerActive(pLayer)) {
      if (IN_RANGE(pEncoderState->CurrentValue, pLayer->StartPosition,
                   pLayer->StopPosition)) {
        // Conversion from encoder value to virtual layer value.
        float percent =
            (float)(pEncoderState->CurrentValue - pLayer->StartPosition) /
            (pLayer->StopPosition - pLayer->StartPosition);

        uint16_t virtualValue = 0;

        // Check if min/max are reversed (i.e the range is counting down)
        if (pLayer->MinValue < pLayer->MaxValue) {
          virtualValue = (uint16_t)(percent * pLayer->MaxValue);
        } else {
          virtualValue = (uint16_t)((1.0f - percent) * pLayer->MinValue);
        }

        MIDI_ProcessLayer(pEncoderState, pLayer,
                          virtualValue >> 9); // TODO - 7-bit lazy conversion is
                                              // bad, what about NRPN/14-bit?
      }
    }
  }
}
...
void Encoder_Update(void) {
  for (int encoder = 0; encoder < NUM_ENCODERS; encoder++) {
    ...
    sEncoderState* pEncoderState =
        &gData.EncoderStates[gData.CurrentBank][(NUM_ENCODERS - 1) - encoder];
    ...
    if (IsAnyLayerActive(pEncoderState)) {
      if (UpdateEncoderRotary(encoder, &gData.HardwareEncoders[encoder],
                              pEncoderState)) {
