...
typedef struct {
	u8						 len_data;
	stream_state_e next_state;
} sysex_type_streamer_def_s;

typedef struct {
	size_t offset;
	size_t len;
} sysex_item_data_info_s;
...
static const stream_state_e sysex_next_state[SYSEX_TYPE_NB] = {
...
// clang-format off
static const sysex_item_data_info_s sysex_data_info[MF_SYSEX_PARAM_NB] = {
	SYSEX_DATA_INFO(MF_SYSEX_PARAM_ENCODER_DETENT, mf_encoder_s, detent),
	SYSEX_DATA_INFO(MF_SYSEX_PARAM_ENCODER_DISPLAY_MODE, mf_encoder_s, display.mode),
	SYSEX_DATA_INFO(MF_SYSEX_PARAM_ENCODER_VMAP_DISPLAY_MODE, mf_encoder_s, display.virtmode),
	SYSEX_DATA_INFO(MF_SYSEX_PARAM_ENCODER_VMAP_MODE, mf_encoder_s, vmap_mode),
	SYSEX_DATA_INFO(MF_SYSEX_PARAM_ENCODER_VMAP_ACTIVE, mf_encoder_s, vmap_active),
	SYSEX_DATA_INFO(MF_SYSEX_PARAM_ENCODER_SWITCH_STATE, mf_encoder_s, sw_state),
	SYSEX_DATA_INFO(MF_SYSEX_PARAM_ENCODER_SWITCH_MODE, mf_encoder_s, sw_mode),
	SYSEX_DATA_INFO(MF_SYSEX_PARAM_ENCODER_SWITCH_PROTO, mf_encoder_s, sw_cfg),
	SYSEX_DATA_INFO(MF_SYSEX_PARAM_VMAP_RANGE, virtmap_s, range),
	SYSEX_DATA_INFO(MF_SYSEX_PARAM_VMAP_POSITION, virtmap_s, position),
	SYSEX_DATA_INFO(MF_SYSEX_PARAM_VMAP_RGB, virtmap_s, rgb),
	SYSEX_DATA_INFO(MF_SYSEX_PARAM_VMAP_RB, virtmap_s, rb),
	SYSEX_DATA_INFO(MF_SYSEX_PARAM_VMAP_PROTO, virtmap_s, cfg),
	SYSEX_DATA_INFO(MF_SYSEX_PARAM_SIDE_SWITCH, mf_rt_s, curr_bank),
	SYSEX_DATA_INFO(MF_SYSEX_PARAM_ACTIVE_BANK, mf_rt_s, curr_bank),
};

// clang-format on

static stream_state_e stream_state = STREAM_IDLE;
...
static int midi_in_handler(void* evt) {
	...
	switch (msg->param_enum) {
		...
		case MF_SYSEX_PARAM_ENCODER_SWITCH_PROTO: {
			...
			mf_encoder_s* encoder = &gENCODERS[bank][enc];
			void*					param =
		...
		case MF_SYSEX_PARAM_VMAP_PROTO: {
			...
			virtmap_s* vmap			= &gENCODERS[bank_idx][enc_idx].vmaps[vmap_idx];
			void*			 param =
