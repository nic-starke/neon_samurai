...
// For more information on CBOR see -
 ...
 *
 * To calculate the required size for Buffer - (sizeof(sMessage) +
 * SIZEOF_CBORLABEL * NUM_LABELS_MESSAGE)
 *
 * @param pMessage A pointer to the message to encode.
 * @param Buffer The UsefulBuf that will store the encoded messages. Must be
 * large enough to store the encoded message.
 * @return UsefulBufC A pointer to the encoded byte stream - will be NULL if
 * encoding failed.
 */
UsefulBufC CBOR_Encode_Message(const sMessage* const pMessage,
                               UsefulBuf             Buffer) {
  ...
  QCBOREncode_AddUInt64ToMap(&ctx, "t",
                             pMessage->Header.Destination.ClientType);
  QCBOREncode_AddUInt64ToMap(&ctx, "a",
