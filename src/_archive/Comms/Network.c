...
static void            SendConnectionRequest(sNetworkAddress* pRemoteDevice,
                                             eCommsProtocol   Protocol);
...
static uint16_t        GetNumberOfConnectedPeers(void);
static bool            MessageHandler(sMessage* pMessage);

/**
 * @brief Initialise the Network module.
 * Recalls the last used network address from EEPROM and assign it to the local
 * address. Registers the Network module with comms.
 */
...
/**
 * @brief To be called in the main loop - performs network updates periodically
 * if required.
 */
...
static void NetworkDiscovery(void) {
  ...
  static uint8_t    attempts       = MAX_DISCOVERY_ATTEMPTS;
 ...
 * If there are more than 16 muffin devices already in the network this function
 * will not set a local address.
 */
static void AddressResolution(void) {
 ...
 * This is a broadcast message that should be sent after a valid network address
 * has been resolved. This will notify other network devices of the local
 * network address.
 */
static void SendDeviceRegistrationMessage(void) {
  sMessage msg                    = MSG_Default;
 ...
 * If NewAddress == UNASSIGNED_ADDRESS then the system will set address state to
 * unitialised, but will not perform address discovery/resolution.
 *
...
static sNetworkAddress GetMuffinEditorAddress(void) {
  ...
  // FIXME - this will return an invalid address, needs to be state aware (i.e
  // has the editor been discovered?)
  return address;
}
 ...
 *
 * @return uint16_t The number of peers, 0 to 16;
 */
static uint16_t GetNumberOfConnectedPeers(void) {
  // Kernighan bit count algorithm
  uint16_t numPeers    = 0;
  uint16_t connections = mConnections.MuffinConnections.Peers;
...
static bool MessageHandler(sMessage* pMessage) {
  ...
  case DISCOVERY_REPLY: {
    ...
    case CLIENT_EDITOR: {
      // FIXME - what happens if there are two editors attempting to send
      // discovery replies?
    ...
    case CLIENT_MUFFIN: {
      // FIXME - should this check a UUID, or possibly a connection sate for
      // this particular peer, before updating the connection state?
      mConnections.MuffinConnections.Peers |=
  ...
  case DEVICE_REGISTRATION: {
    switch (pMessage->Header.Source.ClientType) {
    case CLIENT_MUFFIN: {
      // A muffin is registering itself on the network, reset the local address
      // and attempt network discovery.
      mConnections.MuffinConnections.Peers |=
          pMessage->Header.Source.ClientAddress;
      if (pMessage->Header.Source.ClientAddress == mConnections.LocalAddress) {
        SetLocalAddress(UNASSIGNED_ADDRESS);

        // If there are no connection slots available - do not attempt
        // connection!
