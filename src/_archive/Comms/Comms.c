...
#define MAX_INCOMING_MSGS                                                      \
  (5) // The number of messages in the incoming message queue
#define MAX_MSG_DATA_SIZE                                                      \
  (128) // Maximum number of bytes per message (including CBOR encoding
        // requirements)
...
// All protocols must be added to this register during initial system bootup
// sequence.
struct _protocols {
  bool                          Registered;
...
// All modules must be added to this register during initial system bootup
// sequence.
struct _modules {
...
/**
 * @brief Calls the appropriate SendMessageHandler based on the "Protocol"
 * argument
 *
 ...
 * @return True on successful transmission of byte stream. False on failed
 * transmission of byte stream.
 */
static bool SendMessage(uint8_t* pBytes, uint16_t NumBytes,
 ...
 * Note - only valid CBOR encoded data will be appended - it will not be
 * encoded, do this yourself.
...
bool Comms_SendMessage(sMessage* pMessage) {
  ...
  // #ifdef ENABLE_SERIAL
  //     char buf[192] = "";
  //     sprintf(buf, "SendMessage\r\nProtocol[%d] Priority[%d]
  //     Param[%d]\r\nSource | Type[%d] Address[%d] Module[%d]\r\nDestin |
  //     Type[%d] Address[%d] Module[%d]\r\nVal[%lu]\r\n",
  //         pMessage->Header.Protocol, pMessage->Header.Priority,
  //         pMessage->Header.ModuleParameter,
  //         pMessage->Header.Source.ClientType,
  //         pMessage->Header.Source.ClientAddress,
  //         pMessage->Header.Source.ModuleID,
  //         pMessage->Header.Destination.ClientType,
  //         pMessage->Header.Destination.ClientAddress,pMessage->Header.Destination.ModuleID,
  //         pMessage->Value);
  ...
  UsefulBuf_MAKE_STACK_UB(
      cborBuffer, (sizeof(sMessage) + SIZEOF_CBORLABEL * NUM_LABELS_MESSAGE));
  ...
  if (pMessage->Header.Protocol == PROTOCOL_BROADCAST) {
    ...
    bool success =
        true; // FIXME - this provides no information on which protocol failed,
              // what should be done to recover from failure?
    for (eCommsProtocol protocol = 0; protocol < NUM_COMMS_PROTOCOLS;
         protocol++) {
      if (SendMessage((uint8_t*)encodedMessage.ptr, encodedMessage.len,
                      protocol) == false) {
        success = false;
      }
    }
    ...
    return SendMessage(encodedMessage.ptr, encodedMessage.len,
                       pMessage->Header.Protocol);
  }
 ...
 */
bool Comms_RegisterProtocol(eCommsProtocol                Protocol,
                            fpProtocol_SendMessageHandler SendHandler) {
  ...
  // if(UpdateHandler) // TODO - may use this another time, commented out for
  // now.
 ...
 * @param MessageHandler A pointer to the message handler that will be used when
 * messages are received for this module.
 * @return True if registration successful, False otherwise.
 */
 ...
 * @return uint16_t the total size of the message, including the header, and
 * data.
 */
uint16_t Comms_MessageSize(sMessage* pMessage) {
  return sizeof(sMessageHeader) + pMessage->Data.len;
}

/**
 * @brief Processes an incoming message and passes to the appropriate module
 * message handler.
 *
 * @param pMessage A pointer to the message to process.
 * @return True if module message handler processed the message, false
 * otherwise.
 */
static bool ProcessMessage(sMessage* pMessage) {
  if (pMessage->Header.Destination.ClientAddress != BROADCAST_ADDRESS ||
    ...
    return true; // no need to handle this message because it wasnt for this
                 // unit.
  }
  ...
  if (IsValidModuleID(pMessage->Header.Destination.ModuleID)) {
    if (mModules[pMessage->Header.Destination.ModuleID].fpMessageHandler !=
        NULL)
      return mModules[pMessage->Header.Destination.ModuleID].fpMessageHandler(
          pMessage);
  }
