...
#define OUTBUFFER_LEN                                                           \
	(SIZEOF_SYSEXBLOCK) // a single 8-byte buffer is used and encoded data is
											// effectively "streamed" out
...
static const uint8_t MANF_ID[3] PROGMEM = {
		0x00, 0x48, 0x01}; // TODO is it worth moving this to progmem?
...
static void			DecodeFromSysex(uint8_t* pSysexBytes, uint8_t* pDestBuffer,
																uint16_t NumBlocks);
...
static inline void TransmitMidiNote(uint8_t Channel, uint8_t Note,
 ...
 * It handles transmitting midi values based on a virtual encoder layers current
 * configuration.
 *
...
void MIDI_ProcessLayer(sEncoderState*				 pEncoderState,
	...
	// TODO the ValueToTransmit argument seems very odd here - instead perform a
	// lookup on the hardware encoder and calculate the necessary value?
	switch ((eMidiMode)pLayer->MidiConfig.Mode) {
		case MIDIMODE_CC: {
			TransmitMidiCC(pLayer->MidiConfig.Channel, pLayer->MidiConfig.MidiValue.CC,
										 ValueToTransmit);
			break;
		}
		...
		case MIDIMODE_REL_CC: {
			if (pEncoderState->CurrentValue < pEncoderState->PreviousValue) {
				TransmitMidiCC(pLayer->MidiConfig.Channel,
											 pLayer->MidiConfig.MidiValue.CC, 0x3F);
			} else if (pEncoderState->CurrentValue > pEncoderState->PreviousValue) {
				TransmitMidiCC(pLayer->MidiConfig.Channel,
											 pLayer->MidiConfig.MidiValue.CC, 0x41);
			}
		...
		case MIDIMODE_NOTE: {
			TransmitMidiNote(pLayer->MidiConfig.Channel,
											 pLayer->MidiConfig.MidiValue.Note, ValueToTransmit, true);
			break;
		}
...
// This does not encode data to ensure status bit is valid - ensure data has been
// encoded.
		...
		case MIDI_EVENT(0, MIDI_COMMAND_SYSEX_START_3BYTE): {
			...
			switch (mParserState) {
				case PARSER_INIT: {
					...
					// }
					if (pMsg->Data[2] == mSysexChannel ||
							pMsg->Data[2] == SYSEX_BROADCAST_CH) {
...
/*
		To use sysex messages as an arbitrary data stream it is necessary to
	 encode/decode data. Data bytes cannot have the midi status bit set (bit 8),
	 therefore the maximum value for any byte 0x7F.
 ...
 * @param pDest A destination buffer - must be at least 8 bytes regardless of the
 * NumDataBytes
 * @param DestBufferLen The output buffer length
 * @return uint16_t The number of bytes that were encoded.
 */
static uint16_t EncodeSysexBlock(uint8_t* pData, uint16_t NumDataBytes,
																 uint8_t* pDest, uint8_t DestBufferLen) {
	...
	for (uint8_t i = 0; i < 7; i++) // encode 7 bytes
	{
		...
		{
			if (!!(*pData &
						 0x80)) // check if msb is set in the byte, !! converts to boolean
			{
				msb |= (1U << i); // set the msb bit for this data byte
			}
			*pDest = (*pData & 0x7F); // remove the msb from the data byte then store
...
/**
 * @brief Calculates the number of bytes required to decode from a SysEx encoded
 * stream;
 *
 ...
 */
static inline void TransmitMidiNote(uint8_t _Channel, uint8_t _Note,
																		uint8_t _Velocity, bool _NoteOn) {
