...
/**@{*/
/** Sets the USB bus interrupt priority level to be low priority. The USB bus
 * interrupt is used for Start of Frame events, bus suspend and resume events,
 * bus reset events and other events related to the management of the USB bus.
 */
#define USB_OPT_BUSEVENT_PRILOW ((0 << 2) | (0 << 1))

/** Sets the USB bus interrupt priority level to be medium priority. The USB bus
 * interrupt is used for Start of Frame events, bus suspend and resume events,
 * bus reset events and other events related to the management of the USB bus.
 */
#define USB_OPT_BUSEVENT_PRIMED ((0 << 2) | (1 << 1))

/** Sets the USB bus interrupt priority level to be high priority. The USB bus
 * interrupt is used for Start of Frame events, bus suspend and resume events,
 * bus reset events and other events related to the management of the USB bus.
 */
#define USB_OPT_BUSEVENT_PRIHIGH ((1 << 2) | (0 << 1))

/** Sets the USB controller to source its clock from the internal RC 32MHz
 * clock, once it has been DFLL calibrated to 48MHz. */
