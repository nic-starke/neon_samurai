...
/* Type Defines: */
/** Type define for a endpoint table entry, used to configure endpoints in
 * groups via \ref Endpoint_ConfigureEndpointTable().
 */
typedef struct
{
 ...
 * endpoint's numerical address in the device.
 */
#define ENDPOINT_EPNUM_MASK 0x0F

/** Endpoint address for the default control endpoint, which always resides in
 * address 0. This is defined for convenience to give more readable code when
 * used with the endpoint macros.
 */
