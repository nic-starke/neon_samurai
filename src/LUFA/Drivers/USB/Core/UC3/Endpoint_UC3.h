 ...
 *  \brief Endpoint data read/write definitions for the Atmel AVR32 UC3
 * architecture.
 *
 ...
 *  \defgroup Group_EndpointPrimitiveRW_UC3 Read/Write of Primitive Data Types
 * (UC3) \brief Endpoint primitive read/write definitions for the Atmel AVR32
 * UC3 architecture.
 *
 *  Functions, macros, variables, enums and types related to data reading and
 ...
 *  \defgroup Group_EndpointPacketManagement_UC3 Endpoint Packet Management
 * (UC3) \brief Endpoint packet management definitions for the Atmel AVR32 UC3
 * architecture.
 *
...
/* Inline Functions: */
static inline uint32_t Endpoint_BytesToEPSizeMask(const uint16_t Bytes) ATTR_WARN_UNUSED_RESULT ATTR_CONST ATTR_ALWAYS_INLINE;
...
bool Endpoint_ConfigureEndpoint_Prv(const uint8_t Number, const uint32_t UECFGXData);
 ...
 * 0) which may be used in the device. Different AVR models support different
 * amounts of endpoints, this value reflects the maximum number of endpoints for
 ...
 * the endpoint's index - refer to the chosen microcontroller model's datasheet
 * to determine the maximum bank size for each endpoint. \n\n
 *
 ...
 */
static inline bool Endpoint_ConfigureEndpoint(const uint8_t Address, const uint8_t Type, const uint16_t Size,
                                              const uint8_t Banks) ATTR_ALWAYS_INLINE;
static inline bool Endpoint_ConfigureEndpoint(const uint8_t Address, const uint8_t Type, const uint16_t Size, const uint8_t Banks)
{
    ...
    return Endpoint_ConfigureEndpoint_Prv(
        Number, (AVR32_USBB_ALLOC_MASK | ((uint32_t)Type << AVR32_USBB_EPTYPE_OFFSET) |
 ...
 * are awaiting acknowledgement via the \ref Endpoint_ClearOUT() command.
 *
...
{
    return ((AVR32_USBB.udint & (AVR32_USBB_EP6INT_MASK | AVR32_USBB_EP5INT_MASK | AVR32_USBB_EP4INT_MASK | AVR32_USBB_EP3INT_MASK |
...
static inline void Endpoint_ClearSETUP(void)
{
    ...
    USB_Endpoint_FIFOPos[USB_Endpoint_SelectedEndpoint] =
        &AVR32_USBB_SLAVE[USB_Endpoint_SelectedEndpoint * ENDPOINT_HSB_ADDRESS_SPACE_SIZE];
}
