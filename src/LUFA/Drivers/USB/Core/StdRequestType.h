 ...
 * so that the request details (such as data direction, request recipient, etc.)
 * can be extracted via masking.
 *
 *  @{
 */
...
/* Macros: */
/** Mask for the request type parameter, to indicate the direction of the
 * request data (Host to Device or Device to Host). The result of this mask
 * should then be compared to the request direction masks.
 *
 *  \see \c REQDIR_* macros for masks indicating the request data direction.
 */
#define CONTROL_REQTYPE_DIRECTION 0x80

/** Mask for the request type parameter, to indicate the type of request
 * (Device, Class or Vendor Specific). The result of this mask should then be
 * compared to the request type masks.
 *
 *  \see \c REQTYPE_* macros for masks indicating the request type.
 */
#define CONTROL_REQTYPE_TYPE 0x60

/** Mask for the request type parameter, to indicate the recipient of the
 * request (Device, Interface Endpoint or Other). The result of this mask should
 * then be compared to the request recipient masks.
 *
...
/**@{*/
/** Request data direction mask, indicating that the request data will flow from
 * host to device.
 *
...
/**@{*/
/** Request recipient mask, indicating that the request is to be issued to the
 * device as a whole.
 *
...
/** Request recipient mask, indicating that the request is to be issued to an
...
/** Request recipient mask, indicating that the request is to be issued to an
 * endpoint in the currently selected configuration.
 *
 *  \see \ref CONTROL_REQTYPE_RECIPIENT macro.
 */
...
/** Request recipient mask, indicating that the request is to be issued to an
 * unspecified element in the currently selected configuration.
 *
 *  \see \ref CONTROL_REQTYPE_RECIPIENT macro.
 */
 ...
 *
 *  \see The USB 2.0 specification for more information on standard control
 * requests.
 */
typedef struct
{
...
/* Enums: */
/** Enumeration for the various standard request commands. These commands are
 * applicable when the request type is \ref REQTYPE_STANDARD (with the exception
 * of \ref REQ_GetDescriptor, which is always handled regardless of the request
 * type value).
 *
...
/** Feature Selector values for Set Feature and Clear Feature standard control
 * requests directed to the device, interface and endpoint recipients.
 */
