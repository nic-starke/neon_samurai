 ...
 *  Device Mode USB Class driver framework interface, for the MIDI USB Class
 * driver.
 *
 *  @{
 */
 ...
 * MIDI interface within the user application, and passed to each of the MIDI
 * class driver functions as the \c MIDIInterfaceInfo parameter. This stores
 * each MIDI interface's configuration and state information.
 */
typedef struct
{
	struct
...
/* Function Prototypes: */
/** Configures the endpoints of a given MIDI interface, ready for use. This
 ...
 * the given MIDI interface is selected.
 *
...
/** General management task for a given MIDI class interface, required for the
...
/** Sends a MIDI event packet to the host. If no host is connected, the event
 * packet is discarded. Events are queued into the endpoint bank until either
 * the endpoint bank is full, or \ref MIDI_Device_Flush() is called. This allows
 * for multiple MIDI events to be packed into a single endpoint packet,
 ...
 * populated \ref MIDI_EventPacket_t structure containing the MIDI event to
 * send.
 *
 ...
 * function's packing behavior, to flush queued events.
 *
 *  \param[in,out] MIDIInterfaceInfo  Pointer to a structure containing a MIDI
 * Class configuration and state.
 *
...
/** Receives a MIDI event packet from the host. Events are unpacked from the
 * endpoint, thus if the endpoint bank contains multiple MIDI events from the
 * host in the one packet, multiple calls to this function will return each
 * individual event.
 *
 *  \pre This function must only be called when the Host state machine is in the
 * \ref HOST_STATE_Configured state or the call will fail.
 ...
 * given MIDI class interface. This should be linked to the library \ref
