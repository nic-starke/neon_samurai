 ...
 *  Device Mode USB Class driver framework interface, for the Mass Storage USB
 * Class driver.
 *
 *  @{
 */
 ...
 * Mass Storage interface within the user application, and passed to each of the
 * Mass Storage class driver functions as the \c MSInterfaceInfo parameter. This
...
/* Function Prototypes: */
/** Configures the endpoints of a given Mass Storage interface, ready for use.
 * This should be linked to the library \ref
 * EVENT_USB_Device_ConfigurationChanged() event so that the endpoints are
 * configured when the configuration containing the given Mass Storage interface
 * is selected.
 *
 *  \param[in,out] MSInterfaceInfo  Pointer to a structure containing a Mass
 ...
 * given Mass Storage class interface. This should be linked to the library \ref
...
/** General management task for a given Mass Storage class interface, required
 ...
 *
 *  \param[in,out] MSInterfaceInfo  Pointer to a structure containing a Mass
 * Storage configuration and state.
 */
void MS_Device_USBTask(USB_ClassInfo_MS_Device_t* const MSInterfaceInfo) ATTR_NON_NULL_PTR_ARG(1);

/** Mass Storage class driver callback for the user processing of a received
 * SCSI command. This callback will fire each time the host sends a SCSI command
 * which requires processing by the user application. Inside this callback the
 * user is responsible for the processing of the received SCSI command from the
 * host. The SCSI command is available in the CommandBlock structure inside the
 * Mass Storage class state structure passed as a parameter to the callback
 * function.
 *
 *  \param[in,out] MSInterfaceInfo  Pointer to a structure containing a Mass
 * Storage Class configuration and state.
 *
 *  \return Boolean \c true if the SCSI command was successfully processed, \c
 * false otherwise.
 */
