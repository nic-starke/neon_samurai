...
void RNDIS_Device_ProcessRNDISControlMessage(USB_ClassInfo_RNDIS_Device_t* const RNDISInterfaceInfo)
{
	...
	switch (le32_to_cpu(MessageHeader->MessageType))
	{
		case REMOTE_NDIS_INITIALIZE_MSG:
			...
			RNDIS_Initialize_Message_t*	 INITIALIZE_Message	 = (RNDIS_Initialize_Message_t*)RNDISInterfaceInfo->Config.MessageBuffer;
			RNDIS_Initialize_Complete_t* INITIALIZE_Response = (RNDIS_Initialize_Complete_t*)RNDISInterfaceInfo->Config.MessageBuffer;
		...
		case REMOTE_NDIS_QUERY_MSG:
			...
			void* QueryData = &RNDISInterfaceInfo->Config
								   .MessageBuffer[sizeof(RNDIS_Message_Header_t) + le32_to_cpu(QUERY_Message->InformationBufferOffset)];
		...
		case REMOTE_NDIS_SET_MSG:
			...
			void* SetData = &RNDISInterfaceInfo->Config
								 .MessageBuffer[sizeof(RNDIS_Message_Header_t) + le32_to_cpu(SET_Message->InformationBufferOffset)];

			SET_Response->Status =
				RNDIS_Device_ProcessNDISSet(RNDISInterfaceInfo, SET_Oid, SetData, le32_to_cpu(SET_Message->InformationBufferLength))
		...
		case REMOTE_NDIS_KEEPALIVE_MSG:
			...
			RNDIS_KeepAlive_Message_t*	KEEPALIVE_Message  = (RNDIS_KeepAlive_Message_t*)RNDISInterfaceInfo->Config.MessageBuffer;
			RNDIS_KeepAlive_Complete_t* KEEPALIVE_Response = (RNDIS_KeepAlive_Complete_t*)RNDISInterfaceInfo->Config.MessageBuffer;
										...
										const uint16_t SetSize)
{
	...
	switch (OId)
	{
		case OID_GEN_CURRENT_PACKET_FILTER:
			...
			RNDISInterfaceInfo->State.CurrRNDISState =
				(RNDISInterfaceInfo->State.CurrPacketFilter) ? RNDIS_Data_Initialized : RNDIS_Initialized;
...
{
	if ((USB_DeviceState != DEVICE_STATE_Configured) || (RNDISInterfaceInfo->State.CurrRNDISState != RNDIS_Data_Initialized))
	{
		return false;
	}
