    ...
    if (Endpoint_IsOUTReceived())
    {
        ...
        switch (CCIDHeader.MessageType)
        {
            case CCID_PC_to_RDR_IccPowerOn:
            {
                ...
                Status = CALLBACK_CCID_IccPowerOn(CCIDInterfaceInfo, ResponseATR->CCIDHeader.Slot, (uint8_t*)ResponseATR->Data, &AtrLength,
                                                  &Error);
            ...
            case CCID_PC_to_RDR_SetParameters:
            {
                ...
                if (ProtocolNum == CCID_PROTOCOLNUM_T0)
                {
                    if (CCIDHeader.Length * sizeof(uint8_t) == sizeof(USB_CCID_ProtocolData_T0_t))
                    {
                        ...
                        Status = CALLBACK_CCID_SetParameters_T0(CCIDInterfaceInfo, CCIDHeader.Slot, &Error,
                                                                (USB_CCID_ProtocolData_T0_t*)RequestBuffer);
                        if (CCID_CheckStatusNoError(Status))
                        {
                            ResponseParametersStatus->CCIDHeader.Length = CCIDHeader.Length;
                            Status = CALLBACK_CCID_GetParameters_T0(CCIDInterfaceInfo, CCIDHeader.Slot, &Error,
                                                                    &ResponseParametersStatus->ProtocolNum,
                                                                    (USB_CCID_ProtocolData_T0_t*)&ResponseParametersStatus->ProtocolData);
                        }
                    }
                    else
                    {
                ...
                Endpoint_SelectEndpoint(CCIDInterfaceInfo->Config.DataINEndpoint.Address);
                Endpoint_Write_Stream_LE(ResponseParametersStatus,
                                         sizeof(USB_CCID_BulkMessage_Header_t) + 3 + ResponseParametersStatus->CCIDHeader.Length, NULL);
                Endpoint_ClearIN();
                break;
            }
            ...
            case CCID_PC_to_RDR_GetParameters:
            {
                ...
                Status = CALLBACK_CCID_GetParameters_T0(CCIDInterfaceInfo, CCIDHeader.Slot, &Error, &ResponseParametersStatus->ProtocolNum,
                                                        (USB_CCID_ProtocolData_T0_t*)&ResponseParametersStatus->ProtocolData);
            ...
            case CCID_PC_to_RDR_XfrBlock:
            {
                ...
                Status = CALLBACK_CCID_XfrBlock(CCIDInterfaceInfo, CCIDHeader.Slot, RequestBuffer, CCIDHeader.Length,
                                                (uint8_t*)&ResponseBlock->Data, &ResponseDataLength, &Error);
