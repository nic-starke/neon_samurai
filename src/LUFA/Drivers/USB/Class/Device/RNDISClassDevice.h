 ...
 * module:
 *    - LUFA/Drivers/USB/Class/Device/RNDISClassDevice.c <i>(Makefile source
 * module name: LUFA_SRC_USBCLASS)</i>
 *
 *  \section Sec_USBClassRNDISDevice_ModDescription Module Description
 *  Device Mode USB Class driver framework interface, for the RNDIS USB Class
 * driver.
 *
 *  @{
 */
 ...
 * RNDIS interface within the user application, and passed to each of the RNDIS
 * class driver functions as the \c RNDISInterfaceInfo parameter. This stores
 * each RNDIS interface's configuration and state information.
 */
typedef struct
{
	struct
...
/* Function Prototypes: */
/** Configures the endpoints of a given RNDIS interface, ready for use. This
 ...
 * the given RNDIS interface is selected.
 *
 *  \param[in,out] RNDISInterfaceInfo  Pointer to a structure containing a RNDIS
 ...
 * given RNDIS class interface. This should be linked to the library \ref
...
/** General management task for a given RNDIS class interface, required for the
...
/** Determines if a packet is currently waiting for the device to read in and
 * process.
 *
 ...
 * RNDIS Class configuration and state.
 *
 ...
 * by the device in the nominated buffer.
 *
 *  \pre This function must only be called when the Device state machine is in
 * the \ref DEVICE_STATE_Configured state or the call will fail.
 *
 *  \param[in,out] RNDISInterfaceInfo  Pointer to a structure containing an
 * RNDIS Class configuration and state. \param[out]    Buffer Pointer to a
 ...
 * PacketLength        Pointer to a value indicating the length in bytes of the
 * read packet.
 *
 ...
 *
 *  \param[in,out] RNDISInterfaceInfo  Pointer to a structure containing an
 * RNDIS Class configuration and state. \param[in]     Buffer Pointer to a
