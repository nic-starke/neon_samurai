 ...
 *  Device Mode USB Class driver framework interface, for the CCID USB Class
 * driver.
 *
 *  @{
 */
 ...
 *CCID interface within the user application, and passed to each of the CCID
 *class driver functions as the CCIDInterfaceInfo parameter. This stores each
 *CCID interface's configuration and state information.
 */
typedef struct
{
    struct
...
/* Function Prototypes: */
/** Configures the endpoints of a given CCID interface, ready for use. This
 ...
 *
 *	\param[in,out] CCIDInterfaceInfo	Pointer to a structure containing a CCID
 *Class configuration.
 *
 ...
 *given CCID class interface. This should be linked to the library \ref
...
/** General management task for a given CCID class interface, required for the
 ...
 *  When the ICC is inserted into a slot of a CCID, the CCID can activate the
 *ICC, and the ICC will respond with an ATR (answer to reset)
 *
 *	\param[in,out] CCIDInterfaceInfo	Pointer to a structure containing a CCID
 *Class configuration and state. \param[in]     Slot 				The slot ID
 *currently being powered on.
 *  \param[in,out] Atr					Pointer to an array containing the Power On
 *ATR being sent to the device. \param[out]    AtrSize				The size of
 *the ATR being sent (up to 15 bytes maximum).
 ...
 *	\param[in,out] CCIDInterfaceInfo	Pointer to a structure containing a CCID
 *Class configuration and state. \param[in]     Slot 				The slot ID
 *from which we want to retrieve the status.
 *  \param[out]    Error				The result of the operation, or error.
 *
 *  \return	The command result code.
 */
 ...
 *
 *  \param[in,out] CCIDInterfaceInfo	Pointer to a structure containing a CCID
 * Class configuration, state and protocol
 * data. \param[in]     Slot 				The slot ID from which we want to retrieve
 * the status. \param[out]    Error The result of the operation, or error.
 * \param[out]    T0 					Pointer to a buffer containing the new
 * parameters
 *
 *  \return	The command result code.
 */
 ...
 * data. \param[in]     Slot 				The slot ID from which we want to retrieve
 * the status. \param[out]    Error The result of the operation, or error.
 *  \param[out]    ProtocolNum			The CCID protocol ID of the parameter
 * being retrieved. \param[out]    T0 					Pointer to a buffer
 * where the parameters will be returned
 *
 *  \return	The command result code.
 */
 ...
 *  \param[in,out] CCIDInterfaceInfo	Pointer to a structure containing a CCID
 * Class configuration and state.
 *  \param[in]     Slot 				The slot ID from which we want to retrieve
 * the status. \param[in]     ReceivedBuffer 		Pointer to an array holding
 * the received block of bytes \param[in]     ReceivedBufferSize 	The size of
 * the received block of bytes
 *  \param[out]    SendBuffer 			Pointer to a buffer which will hold the
 * bytes being sent back to the host \param[out]    SentBufferSize 		The size
 * of the block of bytes being sent back to the host \param[out]    Error
 * The result of the operation, or error.
 *
 *  \return	The command result code.
 */
 ...
 *	\param[in,out] CCIDInterfaceInfo	Pointer to a structure containing a CCID
 *Class configuration and state.
 *  \param[in]     Slot 				The slot ID to where the message being
 *aborted was sent to.
 *  \param[in]     Seq 					The current sequence number for this
 *message. Must be checked against the current abort message being sent at the
 *control pipe. \param[out]    Error				The result of the operation,
 *or error.
 *
 *  \return	The command result code.
 */
