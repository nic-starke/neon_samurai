...
uint8_t AOA_Host_ConfigurePipes(USB_ClassInfo_AOA_Host_t* const AOAInterfaceInfo, uint16_t ConfigDescriptorSize, void* ConfigDescriptorData)
{
	...
	if (USB_GetNextDescriptorComp(&ConfigDescriptorSize, &ConfigDescriptorData, DCOMP_AOA_Host_NextAndroidAccessoryInterface) !=
	...
	{
		if (USB_GetNextDescriptorComp(&ConfigDescriptorSize, &ConfigDescriptorData, DCOMP_AOA_Host_NextInterfaceBulkEndpoint) !=
			DESCRIPTOR_SEARCH_COMP_Found)
		{
			return AOA_ENUMERROR_NoCompatibleInterfaceFound;
		}
...
static uint8_t DCOMP_AOA_Host_NextAndroidAccessoryInterface(void* const CurrentDescriptor)
{
	...
	if (Header->Type == DTYPE_Interface)
	{
		...
		if ((Interface->Class == AOA_CSCP_AOADataClass) && (Interface->SubClass == AOA_CSCP_AOADataSubclass) &&
...
uint8_t AOA_Host_SendString(USB_ClassInfo_AOA_Host_t* const AOAInterfaceInfo, const char* const String)
{
	if ((USB_HostState != HOST_STATE_Configured) || !(AOAInterfaceInfo->State.IsActive))
		return PIPE_READYWAIT_DeviceDisconnected;
...
uint8_t AOA_Host_SendByte(USB_ClassInfo_AOA_Host_t* const AOAInterfaceInfo, const uint8_t Data)
{
	if ((USB_HostState != HOST_STATE_Configured) || !(AOAInterfaceInfo->State.IsActive))
		return PIPE_READYWAIT_DeviceDisconnected;
...
static int AOA_Host_putchar(char c, FILE* Stream)
{
