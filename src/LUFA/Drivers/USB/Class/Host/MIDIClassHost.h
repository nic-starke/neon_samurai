 ...
 *  Host Mode USB Class driver framework interface, for the MIDI USB Class
 * driver.
 *
 *  @{
 */
 ...
 * the user application, and passed to each of the MIDI class driver functions
 * as the \c MIDIInterfaceInfo parameter. This stores each MIDI interface's
...
/* Function Prototypes: */
/** Host interface configuration routine, to configure a given MIDI host
 * interface instance using the Configuration Descriptor read from an attached
 * USB device. This function automatically updates the given MIDI Host
 ...
 *
 *  \param[in,out] MIDIInterfaceInfo     Pointer to a structure containing an
 * MIDI Class host configuration and state. \param[in]     ConfigDescriptorSize
 ...
 * Configuration Descriptor.
 *
 *  \return A value from the \ref MIDI_Host_EnumerationFailure_ErrorCodes_t
 * enum.
 */
 ...
 * USB_USBTask().
 *
 *  \param[in,out] MIDIInterfaceInfo  Pointer to a structure containing an MIDI
...
/** Sends a MIDI event packet to the device. If no device is connected, the
 * event packet is discarded.
 *
 ...
 *
 *  \param[in,out] MIDIInterfaceInfo  Pointer to a structure containing a MIDI
 * Class configuration and state. \param[in]     Event              Pointer to a
 * populated USB_MIDI_EventPacket_t structure containing the MIDI event to send.
 *
 ...
 * function's packing behavior, to flush queued events. Events are queued into
 * the pipe bank until either the pipe bank is full, or \ref MIDI_Host_Flush()
 * is called. This allows for multiple MIDI events to be packed into a single
 ...
 *
 *  \param[in,out] MIDIInterfaceInfo  Pointer to a structure containing a MIDI
 * Class configuration and state. \param[out]    Event              Pointer to a
 * USB_MIDI_EventPacket_t structure where the received MIDI event is to be
 * placed.
 *
 *  \return Boolean \c true if a MIDI event packet was received, \c false
 * otherwise.
 */
