 ...
 *  Host Mode USB Class driver framework interface, for the Audio 1.0 USB Class
 * driver.
 *
 *  @{
 */
 ...
 * the user application, and passed to each of the Audio class driver functions
 * as the \c AudioInterfaceInfo parameter. This stores each Audio interface's
...
/* Function Prototypes: */
/** Host interface configuration routine, to configure a given Audio host
 * interface instance using the Configuration Descriptor read from an attached
 * USB device. This function automatically updates the given Audio Host
 ...
 * Audio Class host configuration and state. \param[in]     ConfigDescriptorSize
 ...
 * Configuration Descriptor.
 *
 *  \return A value from the \ref AUDIO_Host_EnumerationFailure_ErrorCodes_t
 * enum.
 */
 ...
 *
 *  \param[in,out] AudioInterfaceInfo  Pointer to a structure containing an
 * Audio Class host configuration and state. \param[in]     EnableStreaming
 * Boolean true to enable streaming of the specified interface, \c false to
 * disable
 *
 ...
 *
 *  \param[in,out] AudioInterfaceInfo  Pointer to a structure containing an
 * Audio Class host configuration and state. \param[in]     DataPipeIndex Index
 * of the data pipe whose bound endpoint is to be altered. \param[in]
 * EndpointProperty    Property of the endpoint to get or set, a value from \ref
 * Audio_ClassRequests_t. \param[in]     EndpointControl     Parameter of the
 * endpoint to get or set, a value from \ref Audio_EndpointControls_t.
 ...
 * retrieved data. \param[in,out] Data                Pointer to a location
 * where the parameter data is stored for SET operations, or where the retrieved
 * data is to be stored for GET operations.
 *
 *  \return A value from the \ref USB_Host_SendControlErrorCodes_t enum.
 */
 ...
 *
 *  \param[in,out] AudioInterfaceInfo  Pointer to a structure containing an
 * Audio Class host configuration and state.
 */
 ...
 * from it, and selects the streaming IN pipe ready for reading.
 *
 ...
 * to be written to it, and selects the streaming OUT pipe ready for writing.
 *
 *  \pre This function must only be called when the Host state machine is in the
 * \ref HOST_STATE_Configured state or the call will fail.
 *
 ...
 * Audio_Host_IsSampleReceived() function to ensure that the correct pipe is
 * selected and ready for data.
 *
 *  \param[in,out] AudioInterfaceInfo  Pointer to a structure containing an
 * Audio Class configuration and state.
 *
 ...
 *
 *  \pre This should be preceded immediately by a call to the \ref
 * Audio_Host_IsReadyForNextSample() function to ensure that the correct pipe is
 * selected and ready for data.
 *
 *  \param[in,out] AudioInterfaceInfo  Pointer to a structure containing an
...
static uint8_t DCOMP_Audio_Host_NextAudioControlInterface(void* CurrentDescriptor) ATTR_WARN_UNUSED_RESULT ATTR_NON_NULL_PTR_ARG(1);
static uint8_t DCOMP_Audio_Host_NextAudioStreamInterface(void* CurrentDescriptor) ATTR_WARN_UNUSED_RESULT ATTR_NON_NULL_PTR_ARG(1);
static uint8_t DCOMP_Audio_Host_NextAudioInterfaceDataEndpoint(void* CurrentDescriptor) ATTR_WARN_UNUSED_RESULT ATTR_NON_NULL_PTR_ARG(1);
#endif
#endif
