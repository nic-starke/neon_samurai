...
uint8_t SI_Host_ConfigurePipes(USB_ClassInfo_SI_Host_t* const SIInterfaceInfo, uint16_t ConfigDescriptorSize, void* ConfigDescriptorData)
{
	...
	{
		if (!(StillImageInterface) || USB_GetNextDescriptorComp(&ConfigDescriptorSize, &ConfigDescriptorData,
																DCOMP_SI_Host_NextSIInterfaceEndpoint) != DESCRIPTOR_SEARCH_COMP_Found)
		{
			if (USB_GetNextDescriptorComp(&ConfigDescriptorSize, &ConfigDescriptorData, DCOMP_SI_Host_NextSIInterface) !=
...
uint8_t DCOMP_SI_Host_NextSIInterface(void* const CurrentDescriptor)
{
	...
	if (Header->Type == DTYPE_Interface)
	{
		...
		if ((Interface->Class == SI_CSCP_StillImageClass) && (Interface->SubClass == SI_CSCP_StillImageSubclass) &&
...
uint8_t SI_Host_SendData(USB_ClassInfo_SI_Host_t* const SIInterfaceInfo, const void* Buffer, const uint16_t Bytes)
{
	uint8_t ErrorCode;
