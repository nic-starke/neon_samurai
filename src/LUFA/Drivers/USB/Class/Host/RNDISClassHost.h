 ...
 * module:
 *    - LUFA/Drivers/USB/Class/Host/RNDISClassHost.c <i>(Makefile source module
 * name: LUFA_SRC_USBCLASS)</i>
 *
 *  \section Sec_USBClassRNDISHost_ModDescription Module Description
 *  Host Mode USB Class driver framework interface, for the Microsoft RNDIS
 * Ethernet USB Class driver.
 *
 *  @{
 */
 ...
 * the user application, and passed to each of the RNDIS class driver functions
 * as the \c RNDISInterfaceInfo parameter. This stores each RNDIS interface's
...
/* Function Prototypes: */
/** Host interface configuration routine, to configure a given RNDIS host
 * interface instance using the Configuration Descriptor read from an attached
 * USB device. This function automatically updates the given RNDIS Host
 ...
 *
 *  \param[in,out] RNDISInterfaceInfo    Pointer to a structure containing an
 * RNDIS Class host configuration and state. \param[in]     ConfigDescriptorSize
 ...
 * Configuration Descriptor.
 *
 *  \return A value from the \ref RNDIS_Host_EnumerationFailure_ErrorCodes_t
 * enum.
 */
 ...
 *
 *  \param[in,out] RNDISInterfaceInfo  Pointer to a structure containing an
 * RNDIS Class host configuration and state.
 *
 *  \return A value from the \ref USB_Host_SendControlErrorCodes_t enum or \ref
 * RNDIS_ERROR_LOGICAL_CMD_FAILED if the device returned a logical command
 * failure.
 */
uint8_t RNDIS_Host_SendKeepAlive(USB_ClassInfo_RNDIS_Host_t* const RNDISInterfaceInfo) ATTR_NON_NULL_PTR_ARG(1);

/** Initializes the attached RNDIS device's RNDIS interface. This should be
 * called after the device's pipes have been configured via the call to \ref
 ...
 *
 *  \param[in,out] RNDISInterfaceInfo  Pointer to a structure containing an
 * RNDIS Class host configuration and state. \param[in]     Oid OID number of
 * the parameter to set. \param[in]     Buffer              Pointer to where the
 * property data is to be sourced from. \param[in]     Length Length in bytes of
 * the property data to sent to the device.
 *
 *  \return A value from the \ref USB_Host_SendControlErrorCodes_t enum or \ref
 * RNDIS_ERROR_LOGICAL_CMD_FAILED if the device returned a logical command
 * failure.
 ...
 *  \param[in,out] RNDISInterfaceInfo  Pointer to a structure containing an
 * RNDIS Class host configuration and state. \param[in]     Oid OID number of
 * the parameter to get. \param[in]     Buffer              Pointer to where the
 * property data is to be written to. \param[in]     MaxLength           Length
 * in bytes of the destination buffer size.
 *
 *  \return A value from the \ref USB_Host_SendControlErrorCodes_t enum or \ref
 * RNDIS_ERROR_LOGICAL_CMD_FAILED if the device returned a logical command
 * failure.
 ...
 * RNDIS Class host configuration and state.
 *
 *  \return Boolean \c true if a packet is waiting to be read in by the host, \c
 * false otherwise.
 */
bool RNDIS_Host_IsPacketReceived(USB_ClassInfo_RNDIS_Host_t* const RNDISInterfaceInfo) ATTR_NON_NULL_PTR_ARG(1);

/** Retrieves the next pending packet from the device, discarding the remainder
 * of the RNDIS packet header to leave only the packet contents for processing
 * by the host in the nominated buffer.
 *
 *  \pre This function must only be called when the Host state machine is in the
 * \ref HOST_STATE_Configured state or the call will fail.
 *
 *  \param[in,out] RNDISInterfaceInfo  Pointer to a structure containing an
 * RNDIS Class host configuration and state. \param[out]    Buffer Pointer to a
 * buffer where the packer data is to be written to. \param[in]     BufferSize
 * Size in bytes of the supplied buffer to store the read packet. \param[out]
 * PacketLength        Pointer to where the length in bytes of the read packet
 * is to be stored.
 *
 ...
 *
 *  \param[in,out] RNDISInterfaceInfo  Pointer to a structure containing an
 * RNDIS Class host configuration and state. \param[in]     Buffer Pointer to a
 * buffer where the packer data is to be read from. \param[in]     PacketLength
...
static uint8_t DCOMP_RNDIS_Host_NextRNDISInterfaceEndpoint(void* const CurrentDescriptor) ATTR_WARN_UNUSED_RESULT ATTR_NON_NULL_PTR_ARG(1);
#endif
#endif
