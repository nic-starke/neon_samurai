...
/** \file
 *  \brief Common definitions and declarations for the library USB Still Image
 * Class driver.
 *
 *  Common definitions and declarations for the library USB Still Image Class
 * driver.
 *
 ...
 * Host modes for the USB Still Image Class.
 *
 *  @{
 */
...
/** Used in the DataLength field of a PIMA container, to give the total
 * container size in bytes for a command container.
 *
 *  \param[in] Params  Number of parameters which are to be sent in the \c Param
 * field of the container.
 */
#define PIMA_COMMAND_SIZE(Params) ((sizeof(PIMA_Container_t) - 12) + ((Params) * sizeof(uint32_t)))

/** Used in the DataLength field of a PIMA container, to give the total
 * container size in bytes for a data container.
 *
 ...
 * interface descriptors relating to the Still Image device class.
 */
...
/** Enums for the possible status codes of a returned Response Block from an
 * attached PIMA compliant Still Image device.
 */
 ...
 *
 *  Type define for a PIMA container, use to send commands and receive responses
 * to and from an attached Still Image device.
 *
