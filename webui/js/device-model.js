...
// firmware yet.
export const MidiMode = Object.freeze({
  DISABLED: 0,
  CC: 1,
  CC_14: 2,
  REL_CC: 3,
...
// 0-255/channel), lit only at dead centre with detent on.
...
function defaultBank() {
  return {
    encoders: Array.from({ length: NUM_ENCODERS }, (_, i) => defaultEncoder(i)),
  };
}
...
export class DeviceModel {
  constructor() {
    ...
    this.sideSwitches = Array.from(
      { length: NUM_SIDE_SWITCHES },
      () => SideSwitchMode.NONE
    );
    this.activeBank = 0;
  ...
  async loadFromDevice(protocol, onProgress) {
    const total =
      NUM_BANKS * NUM_ENCODERS * (4 + NUM_VMAPS_PER_ENCODER * 6) +
      NUM_SIDE_SWITCHES;
    let done = 0;
